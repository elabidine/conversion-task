...
def convert_unit(price: Decimal, from_unit, to_unit=None) -> Decimal:
    ...
    Raises:
        ValueError: If either unit is invalid or conversion is not possible.
    """
    ...
    if isinstance(from_unit, SystemUnit) and isinstance(to_unit, SystemUnit):
        ...
        if from_unit.dimension != to_unit.dimension:
            raise ValueError(f"Cannot convert between units of different dimensions: {from_unit} and {to_unit}.")
        
        # Convert price to target unit
        converted_price = price * Decimal(from_unit.units_to_base) / Decimal(to_unit.units_to_base)
    ...
    raise ValueError(f"Cannot convert between custom units or between system and custom units: {from_unit} and {to_unit}.")
